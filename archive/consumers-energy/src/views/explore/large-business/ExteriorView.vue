<template>
  <section class="ce-large-business-exterior">
    <HeaderProgressBar :percentage="scenePositionProgress" />

    <div class="ce-large-business-exterior__viewport">
      <LocationSelector
        ...
        :dialogueInfoDescription="tutorials?.LocationSelector?.Exterior?.Description"
        :dialogueInfoHeading="tutorials?.LocationSelector?.Exterior?.Heading"
      ...
      >
        <template #hotspotItems>
          <HotspotDefault
            ...
            @hotspot-click="onHotspotBtnClick(index, LOCATION_TYPE_EXTERIOR)"
            heightPerc="48%"
          ...
          <HotspotLink
            v-for="(hotspotLink, index) in model?.Exterior?.HotspotLinks"
        ...
        <template #backgroundAssetImages>
          <AnimInteractive
            v-for="(easterEgg, index) in model?.Exterior?.EasterEggs"
            :key="index"
          ...
          <AnimNonInteractive
            v-for="(animation, index) in model?.Exterior?.Animations"
            :key="index"
            @asset-loading="$emit('asset-loading')"
            @asset-loaded="$emit('asset-loaded')"
        ...
        <template #backgroundSceneImages>
          <BackgroundSceneImage
            v-for="(backgroundSceneImage, index) in model?.Exterior?.BackgroundSceneImages"
            :key="index"
            @asset-loading="$emit('asset-loading')"
            @asset-loaded="$emit('asset-loaded')"
          ...
          <AnimNonInteractive
            ...
            top-percentage="0%"
            type="Anim"
            url="https://scottduncancreative.com/archive/consumers-energy/anims/large-business-exterior__birds.json"
            :zindex="Number(2)"
          />
          ...
          <AnimNonInteractive
            ...
            :depth-X="Number(0)"
            :depth-y="Number(0)"
            height-percentage="46.4%"
            left-percentage="0%"
            top-percentage="0%"
            type="Anim"
            url="https://scottduncancreative.com/archive/consumers-energy/anims/large-business-exterior__plane.json"
            :zindex="Number(1)"
          />
        </template>
      </InteractiveScene>
      ...
      <ButtonListOpen
        class="ce-large-business-exterior__list-open-btn"
    ...
    <OverlaySlideOut :isOverlaySlideOutActive="isOverlaySlideOutActive">
      <template v-if="overlayType === OVERLAY_TYPE_LIST">
        <OverlaySlideOutList
          :heading="model?.Exterior?.Heading"
          :subheading="model?.Exterior?.Subheading"
      ...
      <template v-if="overlayType === OVERLAY_TYPE_PRODUCT">
        <OverlaySlideOutDetails
          ...
          :is-hotspot-active="
            currentHotspotLocation === LOCATION_TYPE_EXTERIOR
    ...
    <SoundAmbient
      url="https://scottduncancreative.com/archive/consumers-energy/sounds/large-business-exterior__ambient.mp3"
      ref="ambientSound"
    />
  </section>
</template>
...
import SoundAmbient from '@/components/SoundAmbient.vue'
...
export default {
  setup() {
    const model = useModelStore().model?.Explore?.LargeBusiness

    if (useHotspotsActiveStore().largeBusinessActiveExteriorArray.length === 0) {
      useHotspotsActiveStore().largeBusinessActiveExteriorArray = new Array(
        model?.Exterior?.Hotspots.length
      ).fill(false)
    }

    const hotspotsExteriorActiveArray = useHotspotsActiveStore().largeBusinessActiveExteriorArray

    if (useHotspotsActiveStore().largeBusinessActiveInteriorArray.length === 0) {
      useHotspotsActiveStore().largeBusinessActiveInteriorArray = new Array(
        model?.Interior?.Hotspots.length
      ).fill(false)
    }

    const hotspotsInteriorActiveArray = useHotspotsActiveStore().largeBusinessActiveInteriorArray

    if (useHotspotsViewedStore().largeBusinessViewedExteriorArray.length === 0) {
      useHotspotsViewedStore().largeBusinessViewedExteriorArray = new Array(
        model?.Exterior?.Hotspots.length
      ).fill(false)
    }

    const hotspotsExteriorViewedArray = useHotspotsViewedStore().largeBusinessViewedExteriorArray

    if (useHotspotsViewedStore().largeBusinessViewedInteriorArray.length === 0) {
      useHotspotsViewedStore().largeBusinessViewedInteriorArray = new Array(
        model?.Interior?.Hotspots.length
      ).fill(false)
    }

    const hotspotsInteriorViewedArray = useHotspotsViewedStore().largeBusinessViewedInteriorArray

    const listItemsPrimaryHeading = useModelStore().model?.Explore?.ExteriorLabel
    const listItemsSecondaryHeading = useModelStore().model?.Explore?.InteriorLabel
  ...
  methods: {
    initScene() {
      this.hotspotEasterEggActiveArray = new Array(this.model?.Exterior?.EasterEggs).fill(false)
      this.scenesViewedStore.largeBusinessExterior = true
    ...
    onExportButtonClick() {
      ...
      this.$emit('on-export-button-click', hotspotsActiveInfoArray, this.model.Exterior.Heading)
    },
    onListOpenBtnClick() {
    ...
    onHotspotBtnClick(index: number, location: string) {
      this.currentHotspotLocation = location
    ...
    hideTutorialHotspot() {
      ...
      this.$refs?.ambientSound?.playSound()
    },
    showTutorialListBtn() {
      if (!this.tutorialsViewedStore.tutorialListBtnViewed) {
  ...
  watch: {
    scenePositionProgress() {
      ...
      if (
        ...
        this.scenesViewedStore.largeBusinessExterior &&
        this.scenesViewedStore.largeBusinessInterior &&
...
.ce-large-business-exterior__viewport {
  flex: 1;
  overflow: hidden;
  position: relative;
  width: 100%;
