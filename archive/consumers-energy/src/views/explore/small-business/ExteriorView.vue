<template>
  <section class="ce-small-business-exterior">
    <HeaderProgressBar :percentage="scenePositionProgress" />

    <div class="ce-small-business-exterior__viewport">
      <LocationSelector
      ...
      >
        ...
        <template #backgroundSceneImages>
          ...
          <AnimNonInteractive
            ...
            :depth-X="Number(0.1)"
            :depth-y="Number(0.01)"
            height-percentage="29%"
            left-percentage="23%"
            top-percentage="0%"
            type="Anim"
            url="https://scottduncancreative.com/archive/consumers-energy/anims/small-business-exterior__birds.json"
            :zindex="Number(2)"
          />
          ...
          <AnimNonInteractive
            ...
            :depth-X="Number(0)"
            :depth-y="Number(0)"
            height-percentage="23.2%"
            left-percentage="0%"
            top-percentage="0%"
            type="Anim"
            url="https://scottduncancreative.com/archive/consumers-energy/anims/small-business-exterior__plane.json"
            :zindex="Number(1)"
          />
        </template>
      </InteractiveScene>
      ...
      <ButtonListOpen
        class="ce-small-business-exterior__list-open-btn"
    ...
    <SoundAmbient
      url="https://scottduncancreative.com/archive/consumers-energy/sounds/small-business-exterior__ambient.mp3"
      ref="ambientSound"
    />
  </section>
</template>
...
export default {
  setup() {
    const model = useModelStore().model?.Explore?.SmallBusiness

    if (useHotspotsActiveStore().smallBusinessActiveExteriorArray.length === 0) {
      useHotspotsActiveStore().smallBusinessActiveExteriorArray = new Array(
        model?.Exterior?.Hotspots.length
      ).fill(false)
    }

    const hotspotsExteriorActiveArray = useHotspotsActiveStore().smallBusinessActiveExteriorArray

    if (useHotspotsActiveStore().smallBusinessActiveInteriorArray.length === 0) {
      useHotspotsActiveStore().smallBusinessActiveInteriorArray = new Array(
        model?.Interior?.Hotspots.length
      ).fill(false)
    }

    const hotspotsInteriorActiveArray = useHotspotsActiveStore().smallBusinessActiveInteriorArray

    if (useHotspotsViewedStore().smallBusinessViewedExteriorArray.length === 0) {
      useHotspotsViewedStore().smallBusinessViewedExteriorArray = new Array(
        model?.Exterior?.Hotspots.length
      ).fill(false)
    }

    const hotspotsExteriorViewedArray = useHotspotsViewedStore().smallBusinessViewedExteriorArray

    if (useHotspotsViewedStore().smallBusinessViewedInteriorArray.length === 0) {
      useHotspotsViewedStore().smallBusinessViewedInteriorArray = new Array(
        model?.Interior?.Hotspots.length
      ).fill(false)
    }

    const hotspotsInteriorViewedArray = useHotspotsViewedStore().smallBusinessViewedInteriorArray
  ...
  methods: {
    initScene() {
      this.scenesViewedStore.smallBusinessExterior = true
  ...
  watch: {
    scenePositionProgress() {
      ...
      if (
        ...
        this.scenesViewedStore.smallBusinessExterior &&
        this.scenesViewedStore.smallBusinessInterior &&
...
.ce-small-business-exterior__viewport {
  flex: 1;
  overflow: hidden;
  position: relative;
  width: 100%;
